 ...
 * @author 308tube
 */
@Path("rs_q02_besieged_reached")
public class Rs_q02_Besieged_Reached{
	...
	public Response returnBrandParts(
		...
		try {
			...
			PreparedStatement  q_stm = conn.prepareStatement(
					" SELECT  " + 
							 " Sum(2_8_0_OnlineQryCnvs_Net_Reached_ByAccessStatus.`Net Beneficiaries Reached`) AS Total_Net_Reached_Besieged,  " + 
							 " Sum(`Net Beneficiaries Reached`/SumOfEstimated_PIN_HTR_subdistrict) AS Net_Reached_Besieged_Percentage " + 
							 " FROM  " + 
							 " QryAccess_Total, " + 
							 " (SELECT  " + 
							 ...
							 " WHERE  " + 
							 " (((QryAccess_Total.Access_Status)='Besieged') AND ((2_8_0_OnlineQryCnvs_Net_Reached_ByAccessStatus.Access_Status)='Besieged'))"
					);
